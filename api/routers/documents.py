...
async def scrape_url_content(url: str, timeout: int = 30) -> tuple:
    ...
    try:
        ...
        raise HTTPException(
            status_code=504, detail=f"URL scraping timed out ({timeout}s)"
        )
    except Exception as e:
...
async def upload_document(
    ...
    title: Optional[str] = Form(
        None, description="Custom title (uses filename if not provided)"
    ),
    ...
    try:
        ...
        logger.info(
            f"📄 Uploaded {doc_title}: {chunk_count} chunks, {len(content)} chars"
        )
...
async def scrape_url(
    ...
    try:
        ...
        logger.info(
            f"🌐 Scraped {doc_title}: {chunk_count} chunks, {len(content)} chars"
        )
...
async def upload_multiple_documents(
    ...
    for file in files:
        try:
            ...
            if extension not in type_map:
                errors.append(
                    {"file": filename, "error": f"Unsupported type: {extension}"}
                )
                continue
